...
[[ "$forward_unit_loaded" == false ]] || {
    printf 'previous Lab 5 loopback forward service did not unload\n' >&2
    exit 1
}
...
[[ "$internal_ready" == true ]] || {
    printf 'Lab 5 internal Streamlit health did not become ready\n' >&2
    exit 1
}

systemd-run --user --unit=openshell-lab5-forward --collect -- \
    openshell forward service openshell-lab5 \
    --target-port 8501 --local 127.0.0.1:18501 >/dev/null
...
for _ in $(seq 1 30); do
    if curl --silent --fail --max-time 5 \
        http://127.0.0.1:18501/_stcore/health >/dev/null; then
        host_ready=true
        break
    fi
    sleep 1
...
[[ "$host_ready" == true ]] || {
    printf 'Lab 5 loopback Streamlit health did not become ready\n' >&2
    exit 1
}

printf 'lab5 Streamlit is ready on host loopback 127.0.0.1:18501\n'
