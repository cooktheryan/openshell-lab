"""Validate the effective OpenShell policy recorded by the Lab 5 verifier."""
...
def validate_effective_policy(document: dict) -> None:
    """Reject an ineffective or broader-than-approved Lab 5 policy."""
    if document.get("status") != "effective":
        raise ValueError("Lab 5 policy is not effective")
    policy = document.get("policy")
    if not isinstance(policy, dict):
        raise ValueError("Lab 5 effective policy is missing")

    filesystem = policy.get("filesystem_policy")
    if not isinstance(filesystem, dict):
        raise ValueError("Lab 5 filesystem policy is missing")
    if filesystem.get("include_workdir") is not False:
        raise ValueError("Lab 5 workdir is not confined")
    if filesystem.get("read_write") != REQUIRED_READ_WRITE:
        raise ValueError("Lab 5 writable paths exceed the approved runtime paths")
    read_only = filesystem.get("read_only")
    if not isinstance(read_only, list) or set(read_only) != REQUIRED_READ_ONLY:
        raise ValueError("Lab 5 read-only paths differ from the approved set")

    if policy.get("landlock", {}).get("compatibility") != "hard_requirement":
        raise ValueError("Lab 5 Landlock policy is not hard-required")
    process = policy.get("process")
    if not isinstance(process, dict):
        raise ValueError("Lab 5 process policy is missing")
    if process.get("run_as_user") != "1500" or process.get("run_as_group") != "1500":
        raise ValueError("Lab 5 process identity is not 1500:1500")
    ...
    if policy.get("network_policies", {}) != {}:
        raise ValueError("Lab 5 effective policy grants ordinary network access")
